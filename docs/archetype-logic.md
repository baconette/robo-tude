...
- **Define the "agree most / diverge most" comparison metric.** The wireframe for the two-person comparison view (docs/wireframes-v01/06-comparison-spectrum.svg) surfaces a per-domain "you agree most on X, diverge most on Y" insight computed as a simple `|average-you − average-them|` per domain — this is a distinct metric from the single-visitor standout logic above and isn't defined anywhere in this doc. Needs a decision from whoever owns this scoring logic before that screen can be built.
