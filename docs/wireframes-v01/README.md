...
| 3 | [03-rating-card.svg](03-rating-card.svg) | The core rating interaction — domain badge, use case text, request-explanation (now directly under the use case text), a 10-section segmented progress bar (one section per card in the bundle), and the 1-5 scale. Shows default, selected (auto-advance), and error states |
